...
const X402_PRICES = {
  '/api/strategy': { amount: '50000', display: '$0.05', desc: 'AI Meme Hunter — natural language meme coin analysis with 8 OKX OnchainOS tools' },
};
...
app.get('/api/strategies', async (_req, res) => {
  res.json({
    product: 'AutoYield',
    tagline: 'AI Meme Hunter on X Layer',
    strategies: [
      {
        id: 'custom',
        name: 'AI Meme Hunter',
        description: '用自然语言描述你的 meme 币筛选标准，AI 并行调用 8 个 OKX 链上数据 API 筛选、分析、推荐',
        price: '$0.05 USDC per scan (x402, zero gas)',
        skills: [
          'okx-dex-trenches (meme token discovery)',
          'okx-dex-signal (smart money tracking)',
          'okx-dex-token (5 tools: info, advanced, holders, top traders, cluster)',
          'okx-security (honeypot / rug pull scan)',
          'okx-x402-payment (payment layer)',
        ],
        example: 'Find Solana meme coins under $500K market cap with high turnover and smart money buying',
      },
    ],
    poweredBy: {
      ai: 'Claude Sonnet 4 (20-round parallel tool_use)',
      skills: '8 OKX OnchainOS Skills actively called per scan',
      wallet: 'OKX Agentic Wallet (TEE-secured)',
      payment: 'x402 (OKX Facilitator, zero gas on X Layer)',
    },
  });
});

// POST /api/strategy/start — AI Meme Hunter (x402 paid, $0.05)
app.post('/api/strategy/start', x402Guard('/api/strategy'), express.json(), async (req, res) => {
  const { strategyId = 'custom' } = req.body || {};
  log(`/api/strategy/start: ${strategyId}`);
...
One tag per line. Each must include full contract address and chain ID.`,
  ...
  try {
    strategySkillPrompt = buildSkillPrompt(null, req.body?.rule || '');
    log(`  Strategy prompt: ~${Math.round(strategySkillPrompt.length / 4)} tokens`);
  } catch { strategySkillPrompt = SKILL_SYSTEM_PROMPT; }
...
app.get('/api', (_req, res) => {
  res.json({
    ...
    protocol: 'x402 via OKX Facilitator (zero gas on X Layer)',
    network: 'X Layer (eip155:196)',
    acceptedTokens: ACCEPTED_ASSETS.map(a => ({ symbol: a.symbol, address: a.address })),
    payTo: account.address,
    facilitator: 'OKX OnchainOS',
    endpoints: [
      { method: 'POST', path: '/api/strategy/start', price: '$0.05', desc: 'AI Meme Hunter — natural language meme coin analysis (8 OKX Skills)' },
      { method: 'GET',  path: '/api/strategies',    price: 'free',   desc: 'List available strategies' },
      { method: 'GET',  path: '/api/strategy/status', price: 'free',  desc: 'Portfolio status for a wallet' },
      { method: 'GET',  path: '/status',            price: 'free',   desc: 'Agent health and x402 earnings' },
    ],
    integrations: {
      npm: 'autoyield-meme-scanner (MCP package)',
      referenceClient: 'github.com/wanggang22/autoyield-meme-monitor (Python + Telegram)',
    },
    howToPay: {
      step1: 'POST /api/strategy/start without payment → receive 402 + PAYMENT-REQUIRED header',
      step2: 'Sign EIP-3009 TransferWithAuthorization for 0.05 USDC',
      step3: 'Re-send with X-PAYMENT header (base64 encoded)',
      step4: 'OKX facilitator verifies + settles on X Layer (zero gas)',
      step5: 'Receive 200 + AI analysis + PAYMENT-RESPONSE header with txHash',
    },
  });
});
