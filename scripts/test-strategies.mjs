#!/usr/bin/env node
/**
 * Test AI Meme Hunter end-to-end via /api/strategy/start.
 * Requires AGENT_PRIVATE_KEY env var (X Layer wallet with USDC).
 */
...
async function signX402(req) {
  ...
  return Buffer.from(JSON.stringify({
    ...
    network: a.network || 'eip155:196',
    payload: {
      signature: sig,
      authorization: { from: account.address, to: a.payTo, value: String(a.maxAmountRequired), validAfter: '0', validBefore, nonce, asset: a.asset || USDC },
    },
  })).toString('base64');
}

async function testMemeScan(rule) {
  console.log(`\n${'═'.repeat(60)}\n🧪 AI Meme Hunter — ${rule}\n${'═'.repeat(60)}`);
  const url = `${API}/api/strategy/start`;
  const body = { strategyId: 'custom', rule };
...
const rule = process.argv.slice(2).join(' ') || '找 Solana 上市值 10K-500K、换手率超 50%、前10持仓低于 25% 的搞笑 meme 币';
await testMemeScan(rule);
console.log('\n✨ 测试完成');
